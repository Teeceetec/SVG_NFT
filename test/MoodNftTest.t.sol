...
import {MoodNft} from "../script/MoodNft.sol";
...
contract MoodNftTest is StdCheats, Test {
  ...
  function testInitializedCorrectly () public view {
     assert(
       keccak256(abi.encodePacked(moodNft.name()))== keccak256(abi.encodePacked(NFT_NAME))
     );

     assert(
      keccak256(abi.encodePacked(moodNft.symbol())) == keccak256(abi.encodePacked(NFT_SYMBOL))
     );
  }

  function testCanMintHaveBalnce() public {
    vm.prank(USER);
    moodNft.mintNft();
  ...
  function testTokenURIDefaultIsCorrectlySet() public {
     ...
     asssert(keccak256(abi.encodePacked(moodNft.tokenURI(0))) == keccak256(abi.encodePacked(HAPPY_MOOD_URI)));
  }
  ...
  function testFlipTokenToSad() public {
   ...
   function testEventRecordsCorrectTokenIdOnMinting() public {
    uint256 currentAvailableTokenId = moodNft.getTokenCounter();
     ...
     assert(tokenId, currentAvailableTokenId);
   }
}